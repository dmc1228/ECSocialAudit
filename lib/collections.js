  ...
  if (audit.version != version){
    ...
    forms.forEach(function(form) {
      ...
      if (form.name == 'formA'){
        ...
        var calculatedObj = new Object();
        calculatedObj.id = 'calculated';
        calculatedObj.name = 'Calculated';
        calculatedObj.index = 4;
        calculatedObj.type = 'calculated';
        calculatedObj.operator = 'sum';

        calculatedObj.fields = ['numberOfGirls','numberOfBoys'];
        console.log(form.sections[0].id)
        form.sections[0].sub_sections[1].columns[4] = (calculatedObj);
        calculateTotals(form.sections[0].sub_sections[1], calculatedObj);


        calculatedObj.fields = ['male','female'];
        console.log(form.sections[0].id)
        form.sections[0].sub_sections[2].columns[4] = (calculatedObj);
        calculateTotals(form.sections[0].sub_sections[2], calculatedObj);
      }
...
calculateTotals = function(subsection, calculatedObj) {
  subsection.rows.forEach(function(row) {
    ...
    var diff = inputtedTotalValue-calculatedValue.value;
    row.differenceInCalculations = diff;
